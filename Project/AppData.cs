...
namespace Project
{
    public class AppData
    {
        ...
        public static void CheckAppKey(RRMParaMedicalCollegeEntities dbContext, string key, byte KeyFor)
        {
            bool isValid = false;
        ...
        public static string GenerateStaffCode(RRMParaMedicalCollegeEntities dbContext)
        {
            int SlNo = 0;
            ...
            SlNo += 1;
            return "IC" + SlNo.ToString("D4");
        }

        public static string GeneratePartyCode(RRMParaMedicalCollegeEntities dbContext)
        {
            int SlNo = 0;
            var lastStudent = dbContext.StudentRegistrations
                                       .OrderByDescending(x => x.StudentId)
                                       .FirstOrDefault();

            if (lastStudent != null && !string.IsNullOrEmpty(lastStudent.StudentCode) && lastStudent.StudentCode.Length > 5)
            {
                string numberPart = lastStudent.StudentCode.Substring(5);
                int.TryParse(numberPart, out SlNo);
            }

            SlNo += 1;
            return "RRMPC" + SlNo.ToString("D1"); 
        }
